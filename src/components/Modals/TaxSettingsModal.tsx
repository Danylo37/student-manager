...
import { parseInputToKopiyky, kopiykyToInput } from '@/utils/financials';
...
function TaxSettingsModal() {
  ...
  const [militaryRateStr, setMilitaryRateStr] = useState('1');
  ...
  useEffect(() => {
    if (taxSettings && isOpen) {
      ...
      setMilitaryRateStr(String(taxSettings.military_tax_rate ?? 1));
    }
  }, [taxSettings, isOpen]);
